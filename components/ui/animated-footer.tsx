      ...
      <div className="container mx-auto flex flex-col md:flex-row justify-between w-full gap-4 pb-24 pt-8 px-4">
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-sky-400 transition-colors duration-200">
                  {link.label}
                </a>
              </li>
            ))}
          ...
          <p className="text-sm mt-4 flex items-center gap-x-1">
            <svg className="w-3 h-3" viewBox="0 0 80 80">
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                fill="currentColor"
        ...
        <div className="space-y-4">
          ...
          <div className="text-right mt-4">
            <button 
              onClick={scrollToTop}
              className="text-sm hover:underline inline-flex items-center transition-colors duration-200"
            >
              Back to top
            </button>
          </div>
